...
class CorrelatedChannelLogitsProcessor(LogitsProcessor):
    ...
    def _calc_py_tilde(self, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        for b_idx in range(len(self.bad_words_id_length_1)):
            self.bad_words_mask[b_idx][self.bad_words_id_length_1[b_idx]] = 1
            p1 = torch.sum(p * (1 - self.bad_words_mask[b_idx])).reshape(1)
            py_tilde.append(torch.concatenate([1-p1,p1]))
...
class K_CCWatermarkDetector():
    ...
    def detect(self,
        ...
        for idx, tok_gend in enumerate(input_sequence):
            ...
            # create a mask using partitions: 1) partition into the k sets 2) mask according to the paritions.
            self.bad_words_mask = torch.zeros_like(vocab_permutation)
            partition = [vocab_permutation[i * bl_ct:(i + 1) * bl_ct] for i in range(self.k - 1)]
            partition.append(vocab_permutation[(self.k - 1) * bl_ct:])
